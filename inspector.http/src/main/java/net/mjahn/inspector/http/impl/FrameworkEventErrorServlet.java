        ...
        if (fwEvents.size() == 0) {
            ...
            while (iter.hasNext()) {
                try {
                    ...
                    JobDescription jd = new JobDescriptionBase(null, Activator.getFrameworkInspector().getTrackedBundle(e.getBundle().getBundleId()), Activator.getFrameworkInspector().getTrackedBundle(e.getBundle().getBundleId()), null, e.getThrowable());
                    ReasonerResult result = rsprov.reason(jd);
                    if (result != null) {
                        out.print(getRow(e.getBundle().getBundleId(), getErrorTypeString(((BundleException) e.getThrowable()).getType()), e.getBundle().getSymbolicName(), e.getBundle().getVersion().toString(), result.getConfidenceLevel(), result.getResultMessage()));
                    } else {
                    ...
                    Logger.getLogger(FrameworkEventErrorServlet.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            out.print(FOOTER);
        }
