...
public class CoreHooks implements FindHook, ListenerHook {
	...
	public void find(BundleContext ctx, String serviceName, String filter,
		...
		if(serviceReferences == null || serviceReferences.isEmpty()){
			NotFoundServiceCallImpl nfsc = new NotFoundServiceCallImpl(ctx.getBundle(), serviceName, filter, obtainAll);
			System.out.println("Not satisfied service Request: "+ctx.getBundle().getSymbolicName() + " " + serviceName + " " + filter);
			fwAnalyzer.getTrackedBundleImpl(ctx.getBundle().getBundleId()).addNotFoundServiceCall(nfsc);
		// these services got found
		} else {
			System.out.println("Service lookup succeeded: "+ctx.getBundle().getSymbolicName() + " " + serviceName + " " + filter);
			// do nothing for now
		}
	}
	...
	public void added(Collection coll) {
		...
		while(iter.hasNext()){
			...
			System.out.println("Added Listener for bundle: '"+li.getBundleContext().getBundle().getSymbolicName() + "' with filter: "+ li.getFilter());
	...
	public void removed(Collection coll) {
		...
		while(iter.hasNext()){
			ListenerInfo li = (ListenerInfo) iter.next();
			System.out.println("Remove Listener for bundle '"+li.getBundleContext().getBundle().getSymbolicName() + "' with filter: "+ li.getFilter());
			long id = li.getBundleContext().getBundle().getBundleId();
			TrackedBundleImpl tb = fwAnalyzer.getTrackedBundleImpl(id);
